			...
			<div id="book_box">
			 ...
			 if(!isset($_SESSION['customer_email'])){
				...
				else
				{
					...
					$get_books = "select * from cart where ip_add = '$ip_add'";
	
					$run_get_books = mysqli_query($con,$get_books);
	
					$cust = $_SESSION['customer_email'];
	
					$get_cust = "select * from customer where customer_email = '$cust'";
	
					$run_get_cust = mysqli_query($con, $get_cust);
	
					$run_cust = mysqli_fetch_array($run_get_cust);
	
					$c_id = $run_cust['customer_id'];
	
					$date  = date("Y-m-d");
	
					while($run_order = mysqli_fetch_array($run_get_books))
					{
						$b_id = $run_order['b_id'];
						$qty = $run_order['qty'];
		
						$insert_order = "insert into orders (b_id, qty, order_date, c_id) values ('$b_id','$qty','$date','$c_id')";
